...
> **Status:** the immutable data system, baseline trainer, local seed-noise calibration, research-agent contract, versioned experiment records, append-only evidence ledger, protected paired experiment runner, CI checks, and resumable full-baseline runner are implemented. The three-seed, 20M-token parent baseline is complete. The automatic PatchRCT promotion controller is not integrated yet, and no candidate-patch improvement is claimed.
